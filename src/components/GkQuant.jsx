...
// el generador directo son los TIROS DESVIADOS del rival.
...
export default function GkQuant({ minuto, goalDiff, gkAc, gkH, gkA, offAcum, fuente, snaps, preA, preB, league, matchInfo, homeName, awayName, reds = null }) {
  ...
  const edge = useMemo(() => {
    ...
    return gkEdge({ model, line: l, oddsOver: oO, oddsUnder: isNaN(oU) ? null : oU, confidence: conf.score })
  }, [model, line, oddsOver, oddsUnder, conf.score])

  const factores = useMemo(() => gkFactores({ model, prior, goalDiff }), [model, prior, goalDiff])
  ...
  useEffect(() => {
    ...
    reportarOportunidad(matchInfo.id, 'gk', edge ? {
      marketBase: 'gk', label: `Saques portería ${edge.lado ? edge.lado + ' ' : 'O/U '}${edge.line}`,
    ...
    if (edge) logDecision({
      ...
      market: 'gk', line: edge.line, odds: edge.oddsOver,
  ...
  }, [edge, conf.score]) // eslint-disable-line react-hooks/exhaustive-deps

  if (gkAc == null) return (
    <div className="card border border-teal-800/40">
      <p className="font-bold text-teal-300 text-sm">🥅 SAQUES DE PORTERÍA — módulo cuantitativo</p>
      <p className="text-xs text-gray-500 mt-1">Este partido aún no reporta saques de portería en vivo (Live-Score nunca los da; Sofascore a veces sí). Puedes ingresarlos a mano arriba en "Stats Acumuladas" y el módulo se activa.</p>
    </div>
  )
  ...
  const nivelOff = model.off?.raw != null ? nivel(model.off.raw) : null

  return (
    <div className="rounded-2xl border-2 border-teal-700/60 bg-gradient-to-b from-teal-950/40 to-dark-800 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-2 text-center">
        {[
          ['Minuto', `${model.minuto}'`],
          ['Actuales', `${model.acum}${gkH != null ? ` (${gkH}·${gkA})` : ''}`],
          ['Ritmo', `${model.rateObs}/min`],
          ['Expected final', model.expectedFinal, 'text-teal-300 text-xl'],
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px]">
        ...
        {nivelOff && (
          <span className="text-gray-400">Tiros desviados: <strong className={nivelOff[1]}>{nivelOff[0]}</strong> <span className="text-gray-600">({model.off.obs}/min vs {model.off.exp}/min esperado)</span></span>
        )}
        {!prior && <span className="text-yellow-600">Sin prior prepartido — espera a que cargue el historial de ambos equipos</span>}
        {prior && (
          <span className="text-gray-400">Prior: <strong className="text-white">{prior.total}</strong>
            <span className="text-gray-600"> ({homeName?.split(' ')[0] ?? 'local'} saca ~{prior.expA} · {awayName?.split(' ')[0] ?? 'visita'} ~{prior.expB}{prior.empTotal ? ` · mediana empírica ${prior.empTotal}` : ''})</span>
          </span>
        )}
      ...
      <div className="bg-dark-800/80 rounded-xl p-3 space-y-2 border border-dark-600">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ['Línea (ej. 21.5)', line, setLine, '0.5'],
        ...
        {edge && (
          <div className="space-y-2">
            ...
            <div className={`rounded-xl px-4 py-3 text-center font-black text-xl ${
              ...
              {edge.signal !== 'NO BET' ? `${edge.signal === 'BET' ? '✅ BET' : '📝 PAPER BET'} ${edge.lado} ${edge.line} · Calidad ${edge.quality} · EV ${edge.evPct > 0 ? '+' : ''}${edge.evPct}%` : '⛔ NO BET'}
      ...
      {showBt && (
        <div className="text-[11px] bg-dark-800/60 rounded-lg p-2">
          ...
          {bt && (
            <>
              <p className="text-gray-400 mb-1">{bt.matches} partido(s) único(s) · {bt.snapsTotal} snapshots (unidad estadística = PARTIDO) — error por minuto:</p>
