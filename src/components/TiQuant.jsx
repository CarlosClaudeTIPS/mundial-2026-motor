...
export default function TiQuant({ minuto, goalDiff, tiAc, tiH, tiA, fuente, snaps, preA, preB, league, matchInfo, homeName, awayName }) {
  ...
  // Reportar al Opportunity Engine + audit log (§47, §57 del spec maestro)
  useEffect(() => {
    if (!matchInfo?.id) return
    const edgeLado = edge ? (edge.lado === 'UNDER' ? edge.edgeUnder : edge.edgeOver) : null
    reportarOportunidad(matchInfo.id, 'ti', edge ? {
      marketBase: 'ti', label: `Saques banda ${edge.lado ? edge.lado + ' ' : 'O/U '}${edge.line}`,
    ...
    if (edge) logDecision({
      ...
      market: 'ti', line: edge.line, odds: edge.oddsOver,
            ...
            <>
              ...
              {bt.calib?.length > 0 && (
                <div className="mt-1.5">
                  <p className="text-gray-500 mb-1">Calibración — el modelo dice% → ocurre% (snapshots del mismo partido NO son independientes):</p>
